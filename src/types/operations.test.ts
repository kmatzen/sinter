...
describe('operations types', () => {
  ...
  it('primitives have 0 expected children', () => {
    ...
    expect(expectedChildren('revolve')).toBe(0);
  });
  ...
  it('modifiers have 1 expected child', () => {
    ...
    expect(expectedChildren('bend')).toBe(1);
  ...
  it('nodeSummary covers every remaining node kind', () => {
    ...
    expect(nodeSummary(mk('extrude', { depth: 5 }))).toBe('XY 5mm profile');
    expect(nodeSummary(mk('revolve', { axis: 1, angle: 270 }))).toBe('XY / Y 270°');
