...
const ADDITIVE_OPTIONAL_PARAMS: Record<string, Set<string>> = {
  ...
  revolve: new Set(['plane', 'axisEdge']),
};
...
function decodeData(kind: string, input: unknown, path: string, context: Context): Record<string, string> | undefined {
  ...
  const allowed = kind === 'mesh' ? new Set(['meshPositions', 'meshName', 'meshTopology', 'meshImportUnit', 'meshImportOrientation', 'meshImportDeclaredUnit'])
  ...
  for (const [key, value] of Object.entries(raw)) {
    ...
    else if (key === 'meshImportOrientation' && !['x-up', 'y-up', 'z-up'].includes(value)) throw new DocumentDecodeError(`${path}.data.meshImportOrientation is invalid`);
    else if (key === 'meshImportDeclaredUnit' && !['micron', 'millimeter', 'centimeter', 'inch', 'foot', 'meter'].includes(value)) throw new DocumentDecodeError(`${path}.data.meshImportDeclaredUnit is invalid`);
