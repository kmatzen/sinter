...
import type { PolygonProfile, Vec2 } from '../../worker/sdf/profile';
...
const clone = (profile: PolygonProfile): PolygonProfile => ({ outer: profile.outer.map((p) => [...p]), holes: profile.holes.map((loop) => loop.map((p) => [...p])) } as PolygonProfile);
...
export function ProfileEditor({ value, fallback, revolve, validate, onCommit }: Props) {
  ...
  const addVertex = () => {
    ...
    loop.splice(selectedVertexIndex + 1, 0, [(loop[selectedVertexIndex][0] + next[0]) / 2, (loop[selectedVertexIndex][1] + next[1]) / 2]);
  ...
  return <div className="mx-2 rounded p-2 space-y-2" style={{ border: '1px solid var(--border-subtle)' }}>
    <svg aria-label="Profile shape editor" role="img" viewBox={`${bounds.minX - bounds.pad} ${-(bounds.maxY + bounds.pad)} ${bounds.width + 2 * bounds.pad} ${bounds.height + 2 * bounds.pad}`}
      ...
      {loops.map((loop, index) => <g key={index}>
        <polygon points={loop.map(([x, y]) => `${x},${-y}`).join(' ')} fill={index ? 'var(--bg-deep)' : 'var(--accent-subtle)'} stroke="var(--accent)" strokeWidth={Math.max(bounds.width, bounds.height) / 180} />
