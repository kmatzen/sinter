...
export function LegalContent({ kind }: { kind: LegalDocument }) {
  ...
  return (
    <div className="space-y-4 text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
      ...
      <p>Sinter uses browser localStorage for your connected-provider profile and OAuth tokens, saved AI provider settings and API keys, chat text, preferences, and consent choice. It uses IndexedDB for project backups and thumbnail caches, and sessionStorage for short-lived OAuth PKCE state. Clearing Sinter’s site data removes these browser copies.</p>
