    ...
    __publicField(this, "updateMatrixWorld", () => {
      ...
      for (let i = 0; i < handles.length; i++) {
        ...
        if (this.mode === "translate" || this.mode === "scale") {
          ...
          // SINTER FIX: The update loop resets handle.rotation to identity,
          // losing the axis-specific cone orientation. The original flip code
          // only adjusts scale, which can't reverse a cone whose rotation was
          // cleared. Fix: when showing the bwd cone, re-apply its rotation so
          // the tip points in the correct (negative) axis direction, then
          // negate local Y scale to flip the tip outward.
          // SINTER FIX: Always show fwd cone, mirror it when camera flips.
          // The bwd cone has its tip pointing toward origin (wrong direction
          // after mirroring). The fwd cone tip points away from origin, so
          // mirroring it preserves the correct tip direction.
          // SINTER FIX: Hide bwd cones, mirror everything else when camera flips.
          if (handle.tag === "bwd") {
            handle.visible = false;
          } else if (handle.name !== "XYZ" && handle.name !== "XYZE") {
