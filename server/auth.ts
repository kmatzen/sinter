...
function upsertUser(profile: UpsertProfile): User {
  // Sanitize inputs
  ...
  if (existing) {
    db.prepare(
      `UPDATE users SET name = ?, avatar_url = ?, email = ?, last_login = datetime('now'),
       oauth_access_token = ?, oauth_refresh_token = COALESCE(?, oauth_refresh_token), oauth_token_expires_at = ?
       WHERE id = ?`
    ).run(
      profile.name, profile.avatar, profile.email,
      profile.accessToken, profile.refreshToken || null, profile.tokenExpiresAt || null,
      existing.id,
    );
  ...
  const id = uuidv4();
  db.prepare(
    `INSERT INTO users (id, email, name, avatar_url, provider, provider_id, last_login,
     oauth_access_token, oauth_refresh_token, oauth_token_expires_at)
     VALUES (?, ?, ?, ?, ?, ?, datetime('now'), ?, ?, ?)`
  ).run(
    id, profile.email, profile.name, profile.avatar, profile.provider, profile.id,
    profile.accessToken, profile.refreshToken || null, profile.tokenExpiresAt || null,
  );
...
export function setupAuth() {
  ...
  if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
    passport.use(new GoogleStrategy({
      ...
      const tokenExpiresAt = new Date(Date.now() + 3600 * 1000).toISOString(); // Google tokens expire in 1 hour
